  ...
  $id("app").innerHTML = `
    ...
    <header class="site-header">
      <div class="container header-inner">
        ...
        <div class="header-actions"><button class="btn btn-secondary" data-action="toggle-lang">${isRTL() ? "English" : "العربية"}</button><button class="btn btn-primary hide-mobile" data-modal="beta">${copy.join}</button></div>
      </div>
    </header>
    <main>
...
function openInfo(type) {
  ...
  if (type === "pricing") {
    ...
    setTimeout(()=>{ const btn=$id("saveBeta"); if(btn) btn.onclick=()=>{ const email=$id("betaEmail").value; localStorage.setItem("doclingo_beta_email", email); modal(names[type], "✔", `<div class="preview-box"><div>✔ ${isRTL()?"تم حفظ الطلب محليًا كتجربة":"Saved locally for this demo"}</div></div><button class="btn btn-primary full" id="okModal">${t[lang].gotIt}</button>`); setTimeout(()=>{ const ok=$id("okModal"); if(ok) ok.onclick=closeModal; },0); }; },0);
    return;
  } else if (type === "contact") {
    modal(names[type], "💬", `<p class="muted">${isRTL()?"نموذج تجريبي للتواصل. الربط الفعلي لاحقًا.":"Demo contact form. Real submission will be connected later."}</p><input class="modal-input" placeholder="${isRTL()?"بريدك الإلكتروني":"Your email"}"/><textarea class="modal-input textarea" placeholder="${isRTL()?"اكتب رسالتك":"Write your message"}"></textarea><button class="btn btn-primary full" id="okModal">${isRTL()?"إرسال تجريبي":"Demo send"}</button>`);
  } else {
