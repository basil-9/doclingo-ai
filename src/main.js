...
function getCredits() {
  ...
  if (saved === null) {
    localStorage.setItem("doclingo_credits", "5");
    return 5;
  }
...
function renderToolModal(tool) {
  ...
  if (tool.id === "merge-pdf") {
    ...
    fields = `<input class="modal-input" id="toolFile" type="file" accept="application/pdf,.pdf"/>${fileLine(state.file)}${state.file ? `<button class="btn btn-secondary full" id="clearSingleFile">${isRTL()?"مسح الملف":"Clear file"}</button>` : ""}`;
...
function bindToolModal(tool) {
  ...
  const clearSingle = $id("clearSingleFile");
  if (clearSingle) clearSingle.onclick = () => { state.file = null; state.error = ""; renderToolModal(tool); };
