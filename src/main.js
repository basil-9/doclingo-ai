...
function recordToolUse(toolId) {
  ...
  localStorage.setItem("doclingo_tool_uses", String(count));
  ...
  $id("app").innerHTML = `
    ...
    <header class="site-header">
      <div class="container header-inner">
        ...
        <div class="header-actions"><button class="btn btn-secondary" data-modal="usage">${isRTL()?"الرصيد":"Credits"}: ${getCredits()}</button><button class="btn btn-secondary" data-action="toggle-lang">${isRTL() ? "English" : "العربية"}</button><button class="btn btn-primary hide-mobile" data-modal="beta">${copy.join}</button></div>
      </div>
    </header>
    <main>
      ...
      <section class="container growth-section">
        <div class="growth-card"><strong>${isRTL()?"كيف يعمل؟":"How it works"}</strong><p>${isRTL()?"اختر أداة، ارفع ملفك، ثم حمّل النتيجة. أدوات PDF الحالية تعمل محليًا داخل المتصفح.":"Choose a tool, upload your file, then download the result. Current PDF tools run locally in your browser."}</p></div>
        <div class="growth-card"><strong>${isRTL()?"نظام الرصيد القادم":"Upcoming credits"}</strong><p>${isRTL()?"يمكن كسب الرصيد عبر مشاهدة الإعلانات، وسيُستخدم لاحقًا لأدوات الذكاء الاصطناعي مثل التلخيص والترجمة.":"Credits can be earned through rewarded ads and will later power AI tools like summaries and translation."}</p></div>
        <div class="growth-card"><strong>${isRTL()?"جاهز للنمو":"Ready to grow"}</strong><p>${isRTL()?"تم تجهيز نماذج Join Beta وContact وربطها مبدئيًا مع Netlify Forms لتجميع الاهتمام المبكر.":"Join Beta and Contact are prepared for Netlify Forms to collect early interest."}</p></div>
      </section>
    </main>
...
function openInfo(type) {
  ...
  if (type === "usage") {
    ...
    modal(isRTL()?"الرصيد والاستخدام":"Credits & Usage", svgIcon("pricing"), `<p class="muted">${isRTL()?"يمكن كسب رصيد إضافي عبر مشاهدة إعلان تجريبي. الربط الإعلاني الحقيقي سيتم لاحقًا.":"Earn extra credits by watching a demo ad. Real ad integration will be connected later."}</p><div class="usage-meter"><div><strong>${credits}</strong><span>${isRTL()?"رصيد حالي":"current credits"}</span></div><div><strong>${used}</strong><span>${isRTL()?"عمليات مستخدمة":"actions used"}</span></div></div><button class="btn btn-primary full" id="watchAdCredit">${isRTL()?"شاهد إعلان واحصل على +2":"Watch ad and earn +2"}</button><button class="btn btn-secondary full" id="resetUsage">${isRTL()?"إعادة ضبط التجربة":"Reset demo"}</button><button class="btn btn-secondary full" id="okModal">${t[lang].gotIt}</button>`);
    setTimeout(()=>{ const ad=$id("watchAdCredit"); if(ad) ad.onclick=()=>startRewardedAd(null, false); const reset=$id("resetUsage"); if(reset) reset.onclick=()=>{ localStorage.setItem("doclingo_tool_uses","0"); setCredits(10); closeModal(); render(); }; const ok=$id("okModal"); if(ok) ok.onclick=closeModal; },0);
...
function openTool(id) {
  ...
  currentTool = { id, file: null, files: [], pages: id === "split-pdf" || id === "remove-pages" ? "1" : id === "reorder-pages" ? "1,2,3" : "", angle: "90", start: "1", error: "", busy: false, rewardUnlocked: false };
...
function renderToolModal(tool) {
  ...
  modal(label(tool), tool.icon, `<p class="muted">${desc(tool)}</p>${fields}${errBox(state.error)}<button class="btn btn-primary full" id="processTool">${state.busy ? t[lang].processing : t[lang].process}</button>`);
  bindToolModal(tool);
}
...
function startRewardedAd(tool, shouldContinue) {
  let seconds = 5;
  const title = isRTL() ? "إعلان للحصول على رصيد" : "Rewarded ad for credits";
  const renderAd = () => {
    modal(title, svgIcon("pricing"), `<p class="muted">${isRTL()?"إعلان تجريبي الآن. لاحقًا سيتم استبداله بإعلان حقيقي. كل مشاهدة تضيف +2 Credits.":"This is a demo ad. Later it will be replaced with a real rewarded ad. Every view adds +2 credits."}</p><div class="ad-box"><strong>${seconds}</strong><span>${isRTL()?"ثواني متبقية":"seconds left"}</span></div><button class="btn btn-secondary full" id="cancelAd">${isRTL()?"إلغاء":"Cancel"}</button>`);
    const cancel = $id("cancelAd");
    if (cancel) cancel.onclick = () => { closeModal(); if (tool) renderToolModal(tool); };
  };
  renderAd();
  const timer = setInterval(() => {
    ...
    addCredits(2);
    if (tool && currentTool) {
      currentTool.rewardUnlocked = true;
      processTool(tool);
    } else {
      modal(title, svgIcon("pricing"), `<div class="preview-box"><div>✔ ${isRTL()?"تمت إضافة +2 Credits":"Added +2 credits"}</div></div><button class="btn btn-primary full" id="okModal">${t[lang].gotIt}</button>`);
      setTimeout(()=>{ const ok=$id("okModal"); if(ok) ok.onclick=()=>{ closeModal(); render(); }; },0);
    }
  }, 1000);
}
...
async function processTool(tool) {
  const s = currentTool;
  if (s && !s.rewardUnlocked) { startRewardedAd(tool, true); return; }
  try {
