...
function BetaAccessModal({ lang, onClose }) {
  const isRTL = lang === "ar";
  const [email, setEmail] = useState("");
  const [done, setDone] = useState(false);
  return (
    <ModalShell lang={lang} title={isRTL ? "انضم للنسخة التجريبية" : "Join Beta"} icon={Mail} description={isRTL ? "اترك بريدك لتكون من أوائل المستخدمين عند تفعيل أدوات الذكاء الاصطناعي والدفع." : "Leave your email to get early access when AI tools and payments are enabled."} onClose={onClose}>
      {done ? (
        <>
          <div className="preview-box"><div><CheckCircle2 size={15} /> {isRTL ? "تم حفظ طلبك محليًا كتجربة." : "Your request was saved locally for this demo."}</div></div>
          <button className="btn btn-primary full" onClick={onClose}>{isRTL ? "تمام" : "Done"}</button>
        </>
      ) : (
        <>
          <input className="modal-input" type="email" value={email} onChange={(event) => setEmail(event.target.value)} placeholder={isRTL ? "بريدك الإلكتروني" : "Your email"} />
          <button className="btn btn-primary full" disabled={!email.includes("@")} onClick={() => { localStorage.setItem("doclingo_beta_email", email); setDone(true); }}>{isRTL ? "انضم للقائمة" : "Join list"}</button>
        </>
      )}
    </ModalShell>
  );
...
function PricingModal({ lang, onClose }) {
  const isRTL = lang === "ar";
  const plans = isRTL ? [
    { name: "Free", price: "مجاني", points: ["أدوات PDF داخل المتصفح", "حدود مناسبة للتجربة", "بدون حساب الآن"] },
    { name: "Credits", price: "لاحقًا", points: ["استخدم رصيد للتلخيص والترجمة", "تحكم في التكلفة", "مناسب للطلاب"] },
    { name: "Pro", price: "لاحقًا", points: ["حدود أعلى", "أدوات AI أكثر", "أولوية في المعالجة"] }
  ] : [
    { name: "Free", price: "Free", points: ["Browser PDF tools", "Fair beta limits", "No account required now"] },
    { name: "Credits", price: "Soon", points: ["Use credits for AI summaries and translation", "Control API cost", "Great for students"] },
    { name: "Pro", price: "Soon", points: ["Higher limits", "More AI tools", "Priority processing"] }
  ];
  return (
    <ModalShell lang={lang} title={isRTL ? "الأسعار والرصيد" : "Pricing & Credits"} icon={Sparkles} description={isRTL ? "واجهة مبدئية فقط. الدفع غير مفعل الآن." : "Initial interface only. Payments are not enabled yet."} onClose={onClose}>
      <div className="pricing-grid">
        {plans.map((plan) => <div className="plan-card" key={plan.name}><strong>{plan.name}</strong><span>{plan.price}</span><ul>{plan.points.map((p) => <li key={p}>{p}</li>)}</ul></div>)}
      </div>
...
function ContactModal({ lang, onClose }) {
  ...
  return (
    <ModalShell lang={lang} title={isRTL ? "تواصل معنا" : "Contact"} icon={MessageCircle} description={isRTL ? "نموذج تجريبي للتواصل. الربط الفعلي سيكون لاحقًا عبر Netlify Forms أو قاعدة بيانات." : "Demo contact form. Real submission will be connected later through Netlify Forms or a database."} onClose={onClose}>
      {sent ? <div className="preview-box"><div><CheckCircle2 size={15} /> {isRTL ? "تم تجهيز الرسالة كتجربة." : "Message prepared for the demo."}</div></div> : <>
        <input className="modal-input" placeholder={isRTL ? "بريدك الإلكتروني" : "Your email"} />
        <textarea className="modal-input textarea" placeholder={isRTL ? "اكتب رسالتك" : "Write your message"}></textarea>
        <button className="btn btn-primary full" onClick={() => setSent(true)}>{isRTL ? "إرسال تجريبي" : "Demo send"}</button>
      </>}
      {sent && <button className="btn btn-primary full" onClick={onClose}>{isRTL ? "تمام" : "Done"}</button>}
    </ModalShell>
  );
}
...
export default function App() {
  ...
  return (
    <div className="app" dir={isRTL ? "rtl" : "ltr"}>
      ...
      <footer className="site-footer"><div className="container footer-inner"><span>© 2026 DocLingo AI</span><nav><button onClick={() => setPricingOpen(true)}>Pricing</button><button onClick={() => setInfoType("privacy")}>Privacy</button><button onClick={() => setInfoType("terms")}>Terms</button><button onClick={() => setInfoType("contact")}>Contact</button></nav></div></footer>
      {renderModal()}
      {infoType === "contact" ? <ContactModal lang={lang} onClose={() => setInfoType(null)} /> : <InfoModal type={infoType} lang={lang} onClose={() => setInfoType(null)} />}
      {betaOpen && <BetaAccessModal lang={lang} onClose={() => setBetaOpen(false)} />}
      {pricingOpen && <PricingModal lang={lang} onClose={() => setPricingOpen(false)} />}
    </div>
  );
}
