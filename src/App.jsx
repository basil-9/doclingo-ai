...
const tools = [
  ...
  { id: "merge-pdf", title: "Merge PDF", ar: "دمج PDF", icon: Archive, status: "Available", category: "PDF", desc: "Combine multiple PDFs into one file directly in your browser.", arDesc: "ادمج أكثر من ملف PDF في ملف واحد مباشرة داخل المتصفح." },
...
function InfoModal({ type, lang, onClose }) {
  ...
  const content = {
    privacy: {
      ...
      body: isRTL
        ? ["نستخدم الملفات فقط لتنفيذ الأداة التي تختارها.", "في أداة دمج PDF الحالية تتم المعالجة داخل المتصفح ولا يتم رفع الملفات إلى خادم.", "لا نستخدم محتوى ملفاتك لتدريب النماذج.", "قد يتم استخدام خدمات معالجة خارجية عند تفعيل أدوات الذكاء الاصطناعي، وسيتم توضيح ذلك قبل الإطلاق الرسمي."]
        : ["We use uploaded files only to perform the tool you choose.", "The current Merge PDF tool runs in the browser and does not upload files to a server.", "We do not use your document content to train models.", "External processing providers may be used when AI tools are activated, and this will be clarified before public launch."],
    },
    terms: {
      icon: FileText,
      title: isRTL ? "شروط الاستخدام" : "Terms of Use",
      body: isRTL
        ? ["DocLingo AI حاليًا في نسخة تجريبية، وبعض الأدوات قد تكون محدودة أو قيد التفعيل.", "لا ينبغي الاعتماد على مخرجات الذكاء الاصطناعي أو التحويلات للاستخدام الرسمي دون مراجعة بشرية.", "يجب ألا ترفع ملفات لا تملك حق استخدامها أو معالجتها.", "قد تتغير الحدود والميزات والأسعار عند الانتقال من النسخة التجريبية إلى النسخة الرسمية."]
        : ["DocLingo AI is currently in beta, and some tools may be limited or under activation.", "AI and conversion outputs should be reviewed before official use.", "Do not upload files you do not have rights to use or process.", "Limits, features, and pricing may change when moving from beta to production."],
    },
    contact: {
      icon: MessageCircle,
      title: isRTL ? "تواصل معنا" : "Contact Us",
      body: isRTL
        ? ["للاقتراحات أو الإبلاغ عن مشكلة، استخدم زر الانضمام للنسخة التجريبية.", "سنضيف نموذج تواصل كامل في المرحلة القادمة."]
        : ["For suggestions or issue reports, use the beta access form.", "A full contact form will be added in the next phase."],
    },
  }[type];
  ...
  return (
    <div className="modal-backdrop" onClick={onClose}>
      <div className="modal" dir={isRTL ? "rtl" : "ltr"} onClick={(e) => e.stopPropagation()}>
        ...
        <h2>{content.title}</h2>
        <ul className="info-list">{content.body.map((item, index) => <li key={index}>{item}</li>)}</ul>
...
function MergePdfModal({ lang, onClose }) {
  ...
  async function mergeFiles() {
    setError("");
    if (files.length < 2) {
      setError(isRTL ? "اختر ملفين PDF على الأقل." : "Choose at least two PDF files.");
      return;
    }
    try {
      setBusy(true);
      ...
      for (const file of files) {
        if (file.type !== "application/pdf" && !file.name.toLowerCase().endsWith(".pdf")) {
          throw new Error(isRTL ? "يُسمح بملفات PDF فقط." : "Only PDF files are allowed.");
        }
      ...
      a.href = url;
      a.download = "doclingo-merged.pdf";
      document.body.appendChild(a);
      a.click();
      a.remove();
      URL.revokeObjectURL(url);
    } catch (err) {
      setError(err.message || (isRTL ? "تعذر دمج الملفات." : "Could not merge the files."));
    } finally {
      setBusy(false);
    }
  }
  ...
  return (
    <div className="modal-backdrop" onClick={onClose}>
      <div className="modal" dir={isRTL ? "rtl" : "ltr"} onClick={(e) => e.stopPropagation()}>
        <button className="modal-close" onClick={onClose}><X size={18} /></button>
        <div className="modal-icon"><Archive size={24} /></div>
        <h2>{isRTL ? "دمج PDF" : "Merge PDF"}</h2>
        <p className="muted">{isRTL ? "اختر ملفين PDF أو أكثر وسيتم دمجها مباشرة داخل المتصفح بدون رفعها إلى خادم." : "Choose two or more PDF files. They will be merged directly in your browser without server upload."}</p>
        <input className="modal-input" type="file" accept="application/pdf,.pdf" multiple onChange={(e) => setFiles(Array.from(e.target.files || []))} />
        {files.length > 0 && <div className="file-list">{files.map((file, idx) => <div key={file.name + idx}><CheckCircle2 size={15} /> {file.name}</div>)}</div>}
        {error && <div className="error-note">{error}</div>}
        <button className="btn btn-primary full" onClick={mergeFiles} disabled={busy}>{busy ? (isRTL ? "جاري الدمج..." : "Merging...") : (isRTL ? "دمج وتحميل" : "Merge and download")}</button>
      </div>
    </div>
  );
}
...
function ToolCard({ tool, lang, onTry, onJoinBeta }) {
  ...
  return (
    <article className="tool-card fade-in">
      <div className="tool-header"><div className="tool-icon"><Icon size={20} /></div><span className={`status ${tool.status.toLowerCase().replaceAll(" ", "-")}`}>{tool.status}</span></div>
      <div className="tool-body"><h3>{lang === "ar" ? tool.ar : tool.title}</h3><p>{lang === "ar" ? tool.arDesc : tool.desc}</p></div>
